...
export class DependencyTracker {
    ...
    */
    ...
    addUserTaintedNodeEdge(nodeId: number, stmtId: number, index: number, isTainted: boolean = true, createSourceEdge: boolean = true): void {
        if (isTainted) this.graph.addEdge(this.graph.taintNode, nodeId, { type: "PDG", label: "USER_TAINT" }); // this.create source edge
        createSourceEdge && this.graphCreateSourceEdge(stmtId, nodeId, index !== undefined ? index : -1); // sources from argv e.g. do not connect as param edge?
    ...
    addUserExportedParams():void {
        const functionDeclarationNodes: Array<GraphNode | undefined> | undefined = this.graph.startNodes.get("CFG")
            // ?.filter(cfgNode => !cfgNode?.functionName?.match("[v]\\d+")) // Not generated by the normalization)
            ?.map((cfgNode: GraphNode) => this.graph.nodes.get(cfgNode.functionNodeId))
        const funcDeclNodesId : number[] = functionDeclarationNodes?.filter((node: GraphNode | undefined): node is GraphNode => node !== undefined).map((node: GraphNode) => node.id) ?? []

        const exportedFuncNodes : Array<GraphNode> | undefined = functionDeclarationNodes?.filter((fnExpNode: GraphNode | undefined): fnExpNode is GraphNode => fnExpNode !== undefined && fnExpNode.exported)


        exportedFuncNodes?.forEach((fnExpNode: GraphNode | undefined) => {
            if (!fnExpNode || !fnExpNode.exported) return;
            this.parseExportedFunc(fnExpNode, funcDeclNodesId)
        })
    }

    parseExportedFunc(fnExpNode: GraphNode, funcDeclNodesId: number[]): void {

        const object : GraphNode = fnExpNode.edges.filter(edge => edge.type === "REF" && edge.label === "obj").map(edge => edge.nodes[1])[0];

        const proto : GraphNode | undefined = object.edges.filter(edge => edge.type === "PDG" && edge.label === "SO" && edge.objName === "prototype").map(edge => edge.nodes[1])[0];

        if (proto) {
            proto.edges.forEach(edge => {
                if (edge.label === "NV"){
                    edge.nodes[1].edges.forEach(edge => {                       
                        if (edge.type === "PDG" && edge.label === "SO") {
                            edge.nodes[1].edges.forEach(edge => {
                                if (edge.label === "CG" && edge.nodes[1].id !== fnExpNode.id && funcDeclNodesId.includes(edge.nodes[1].id)) {
                                    this.parseExportedFunc(edge.nodes[1], funcDeclNodesId)
                                }
                            });
                        }
                    });
        ...
        const astNode: GraphNode = fnExpNode.edges.filter(edge => edge.type === "AST").map(edge => edge.nodes[1])[0];


        
        const blockNode: GraphNode = astNode.edges.filter(edge => edge.type === "AST" && edge.label === "block").map(edge => edge.nodes[1])[0];
        
        blockNode.edges.forEach(edge => {
            if (edge.nodes[1].type === "VariableDeclarator" && edge.nodes[1].id !== fnExpNode.id && funcDeclNodesId.includes(edge.nodes[1].id)) {
                this.parseExportedFunc(edge.nodes[1], funcDeclNodesId)
        ...
        const functionParamNodes: GraphNode[] = fnExpNode.edges.filter(edge =>
                edge.type === "REF" && edge.label === "param")
                .map(edge => edge.nodes[1]);
                
        functionParamNodes.forEach((paramNode: GraphNode, i: number) => {
            // If a param does not have an origin, connect to taint source
            this.addUserTaintedNodeEdge(paramNode.id, paramNode.functionNodeId, i)
        });
    }

       // parseExportedBlock(fnExpNode: GraphNode, blockNode: GraphNode, funcDeclNodesId: number[]): void {

    //     blockNode.edges.forEach(edge => {
    //         if (edge.nodes[1].type === "IfStatement" || edge.nodes[1].type === "WhileStatement" || edge.nodes[1].type === "ForStatement" || edge.nodes[1].type === "SwitchStatement") {
    //             const innerBlockNode: GraphNode | undefined = edge.nodes[1].edges.filter(edge => edge.type === "AST").map(edge => edge.nodes[1])[0];
    //             if (innerBlockNode.type === "BlockStatement") {
    //                 this.parseExportedBlock(fnExpNode, innerBlockNode, funcDeclNodesId)
    //             }
    //         }
    //         else if (edge.nodes[1].type === "VariableDeclarator" && edge.nodes[1].id !== fnExpNode.id && funcDeclNodesId.includes(edge.nodes[1].id)) {
    //             this.parseExportedFunc(edge.nodes[1], funcDeclNodesId)
